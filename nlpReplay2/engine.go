...
func main() {
	...
	trainX, trainY, valX, valY := prepareData(bpe, paras)
	...
	for _, mode := range modes {
		...
		go func(mode string) {
			...
			gen := make(map[string]string, len(demoPrompts))
			for _, p := range demoPrompts {
				gen[p] = generateFromPrompt(net, p, genTokens)
			}
	...
	fmt.Println("\n📊 FINAL SUMMARY")
	final := make(map[string]result, len(modes))
