...
func trainNet[T paragon.Numeric](net *paragon.Network[T], data []sample, cfg trainCfg, wg *sync.WaitGroup) {
	...
	for e := 0; e < cfg.epochs; e++ {
		...
		fmt.Printf("Epoch %02d (%s): loss=%.4f lr=%.4g took=%s\n",
			e, cfg.name, total/float64(len(data)), lr, epochDur)
	}

	totalDur := time.Since(trainStart)
	avgPerEpoch := totalDur / time.Duration(cfg.epochs)
	fmt.Printf("[train done] %s: epochs=%d total=%s avg/epoch=%s\n",
		cfg.name, cfg.epochs, totalDur, avgPerEpoch)
}
...
func main() {
	...
	wg.Wait()
	overallTrainDur := time.Since(overallTrainStart)
	fmt.Printf("\n=== All models trained in %s ===\n", overallTrainDur)
